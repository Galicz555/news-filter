...
const hívd_le_az_oldalt_axiossal = async (
  url: string,
): Promise<AxiosResponse<any, any>> => await axios.get(url);

const szedd_ki_az_axiosból_a_htmlt = async (
  válasz: Promise<AxiosResponse<any, any>>,
): Promise<string> => (await válasz).data;

const töltsd_be_a_htmlt_cheerioval = async (
  html: Promise<string>,
): Promise<cheerio.CheerioAPI> => cheerio.load(await html);
...
const írj_jsont = async (adat: Promise<Csiribá>) =>
  fs.promises.writeFile(
    (await adat).fájlnév,
    JSON.stringify(await adat, null, 2),
  );
